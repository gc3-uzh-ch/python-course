...
\end{lstlisting}
  ...
  \+
  The \texttt{len()} function returns the number of elements in any
...
\begin{frame}[fragile]
  ...
  You can remove individual elements from a list either by specifying
  the element:
\begin{lstlisting}
>>> L.remove('Z')
>>> L
...
Keys must be \textit{hashable} objects.
\end{frame}
